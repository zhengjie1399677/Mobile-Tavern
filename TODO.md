...
| 日期 | 变动内容 |
|---|---|
| 2026-07-17 | 落地 AbortSignal 协作式中断传导与底层资源泄露治理（TODO #5）：`localDB.ts` 引入 `WriteContext` + `bindTransactionAbort` 三路 race（operation / 超时 / signal abort），21 个写函数全部接入 `signal` 参数并主动调用 `transaction.abort()` 释放底层 IDB 资源；`DatabaseService.ts` 6 个方法透传 signal；`types.ts` 更新 `IDatabaseService` / `IScriptService` 接口契约；`mvuParser.ts` 5 个解析函数（extractMvuCommands / extractXmlMvuCommands / detectJsonPatch / parseMvuMessage / applyCharacterRegexScripts）循环顶部插入 `checkAborted` 检查点；`ScriptService.ts` 桥接层透传 signal；`MemoryStreamParser.ts` `onChunk` 循环顶部插入 abort 检查点。新增 `abortSignalConduction.test.ts` 5 项测试覆盖预 abort / 中途 abort / 写队列恢复 / MVU 检查点 / 流式解析器检查点。修复 `mvuScriptIntegration.test.ts` 接口签名期望（bridge 调用新增第三参 signal）。`tsc` + 72/72 测试全绿（含 vitest 327 项，i18n 多语言 50 项已通过 `runVitestSuite` 子进程桥接归纳至主流程）。 |
