...
| 日期 | 变动内容 |
|---|---|
| 2026-07-21 | 修复自由 Prompt 最终预览无法继续下滑的问题：`PromptCompositionPreviewDialog.tsx` 移除单条消息的 `max-h-64` 硬截断，避免内容被卡片 `overflow-hidden` 裁掉；预览消息统一交由带 `touch-pan-y`、`min-h-0` 与 `overflow-y-auto` 的列表容器滚动，竖屏弹层和横屏嵌入工作台共用同一路径。`PromptCompositionEditor.test.tsx` 新增触屏滚动与不截断回归测试。验证：类型检查、定向组件测试、完整测试与生产构建通过。 |
...
| 2026-07-20 | 补全自由 Prompt 编排移动端 UI：`PromptCompositionEditor.tsx` 改为显式传统/自由模式选择与紧凑消息序列，支持触控拖动、上下移动和空发送警告；新增 `PromptBlockEditorDialog.tsx` 底部编辑器与可搜索宏选择器，完整显示历史范围、深度目标、条件和兼容字段；新增 `PromptCompositionPreviewDialog.tsx`，由 `PresetForm.tsx` 调用真实 `PromptService` 预览当前会话最终 `messages`、Token 估算与编译诊断。Token 策略明确标注为仅持久化兼容元数据，未伪装为已执行裁剪。验证：390×844 受控浏览器走查无控制台错误；`npm run lint`、`npm run check:i18n`、`npm test` 80/80、Vitest 336/336、`npm run build` 全部通过，仅保留既有大分块警告。 |
| 2026-07-20 | 增加自由 Prompt 可视化与宽屏配置：新增 `PromptCompositionGraph.tsx`，以只读消息流展示角色、历史范围、深度目标、宏来源、条件、兼容标记和区块诊断；新增 `PromptCompositionWorkbench.tsx` 与 `PromptBlockQuickEditor.tsx`，可用宽度达到 700px 时形成“左侧序列及配置、右侧编排图或真实预览”的双栏工作台，窄屏则把编排图降级为安全区适配的底部面板。图形节点和快速编辑器直接读写同一份 `PromptComposition`，没有平行状态或 UI 自编译；完整高级配置仍复用原底部编辑器。新增 `matchMedia` 响应式、图形定位、视图切换及单一状态源测试。验证：`npm run lint`、`npm run check:i18n`、`npm test` 80/80、Vitest 339/339、`npm run build` 全部通过，仅保留既有大分块警告。 |
| 2026-07-20 | 增加 Android 自由 Prompt 手动横屏入口：`PromptCompositionEditor.tsx` 仅在原生桥接能力存在时显示“进入横屏工作台”，锁定后可原位恢复系统自动旋转；`useAndroidOrientationControl.ts` 隔离平台检测与 UI 状态，浏览器和其他平台不显示入口。`AndroidThemeBridge.kt` 新增 `setScreenOrientation`，使用传感器横屏与 `UNSPECIFIED` 恢复策略，保持 WebView 编辑状态。验证：类型检查与 8 语言一致性通过，Prompt 编辑器测试 9/9，Vitest 341/341，Android `assembleArm64Debug` 通过，`npm test` 80/80 与前端生产构建通过。 |
