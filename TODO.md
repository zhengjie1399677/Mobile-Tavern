...
- [ ] **自定义 Agent 闭环（P0，方向②）**：把「定义 Agent = 选角色 + 挂工具 + 调行为」做成可存、可改、可文件级分享的最小闭环，补齐「Profile 不可导出」这一结构性缺口。两块：①新增 Agent/Profile 文件级导出粘合剂（含角色引用 + 工具挂载 + 行为/采样配置 + 能力开关，导入导出经统一校验、不丢 API 凭证）；②把当前术语墙（driver/llm.route/tool 拼装口）收敛为「选角色 + 勾工具 + 调行为 + 导出」的新手引导式表单，硬核入口保留可开。验收：同一份定义可在另一台设备复现完整 Agent，普通用户不查文档能完成一次定义与导出。
