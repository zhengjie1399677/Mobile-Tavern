...
graph TD
    subgraph Client [Tauri 客户端]
        ...
        API_Client -->|4. 状态机驱动| SVG[SVG 猫耳动画组件]
...
### 4. 视觉表现：SVG 矢量微动画与状态机
*   **组件设计**：为了保障极致的运行性能和响应速度，并且严格遵守 `AGENTS.md` 对网络代理/CDN 依赖的物理隔离限制，猫娘形象首选采用 **React 控制的 SVG 矢量图形与 CSS 微动画** 表达动态情绪：
    *   **猫耳/眨眼/呼吸特效**：用 SVG 路径定义，在不同情绪（闲置 `idle`、思考 `thinking`、说话 `talking`、难过 `sad`）下通过改变 CSS transition 动画和路径（例如耳朵抖动、眼睛弯曲度）来实现极简现代的微表情切换。
    *   **降级兜底**：保留降级加载本地轻量级 WebP 图片的能力，以备后期美术调整。
