...
* 可用宽度达到 700px 时自动进入双栏工作台：左侧保留消息序列和选中区块的快速配置，右侧常驻只读编排图或真实最终消息预览。窄屏仍使用底部面板；图形节点只定位同一份 `PromptComposition`，不会产生第二套编排状态。Android 原生包额外显示“进入横屏工作台”按钮，可直接锁定横屏并随时恢复系统自动旋转；浏览器与其他平台不显示该入口。
...
* 当前主测试链包含 80 组功能套件与 341 项 Vitest 断言，覆盖物理 PNG 解码、SSRF 防御、自由 Prompt 编排与兼容防腐、Android 横屏入口、状态 Schema 迁移、重启消息顺序、IndexedDB 分支尾部原子替换、十轮折叠边界重发、SafeProxy、洋葱管道严格模式及架构依赖边界。
