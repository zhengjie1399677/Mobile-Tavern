# 🐱 内置“猫咪机器人”助手技术方案建议书 (共识规格版)

本文件归档了经过 `/grill-me` 访谈后，双方针对 **Mobile Tavern** 中内置“猫咪助手”功能所达成的最终技术共识。该规格书为接下来的具体实施提供了底层设计指导。
...
*   **物理位置约束**：为了绝对不干扰核心主功能，猫咪桌宠将采用**“特定页面限时现身”**策略：
...
*   **动态生成逻辑**：客户端将这些行为事件作为环境上下文发送至云端，由云端 LLM 实时生成一句富有猫咪性格（带“喵~”、傲娇或可爱语气）的 30 字以内台词。
...
*   **组件设计**：为了呈现高精度的猫咪动漫立绘，并严格遵守 `AGENTS.md` 对网络代理/CDN 依赖的物理隔离限制，猫咪形象将弃用简陋 of SVG 路径绘制，全面采用 **本地打包的轻量级 PNG/WebP 立绘图片 + CSS 物理动效**：
    *   **多状态表情切图**：在本地存放 4 张轻量级的表情立绘（如 `/public/assets/cat/`），对应猫咪的 4 种核心情绪状态：
        *   `idle.png`：闲置/待机状态（温和微笑着的猫咪）。
...
*   **[NEW]** `src/components/FloatingCat.tsx`：猫咪桌宠组件与气泡对话框。
*   **[MODIFY]`src/components/MainLayout.tsx`**：在顶层布局中，根据当前激活的 Tab 路由决定是否渲染 `FloatingCat`。
*   **[NEW]`src/hooks/useCatbot.ts`**：管理猫咪当前的状态机（情绪、文字队列、主动触发计时器）。
*   **[MODIFY]`src/utils/apiClient.ts`**：添加指向阿里云 FC 代理微服务的独立请求分支，接入 STS 签名头。
*   **[NEW]** 云端阿里云 FC 上的猫咪中转函数（独立于客户端仓库之外，独立部署）。
