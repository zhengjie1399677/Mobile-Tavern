...
it.each([false, true])("传统 Prompt 在 roleplayMode=%s 时按历史深度筛选正则", async roleplayMode => {
  ...
  runtime.registerTransform({
    id: "test.regex",
    version: "1.0.0",
    transform: request => applySillyTavernRegexScripts(
      request.text,
      request.character,
      request.isAiMessage,
      request.charName,
      ...
      "prompt",
      request.signal,
      { presetRegexScripts: request.presetRegexScripts, depth: request.depth },
    ),
  });
