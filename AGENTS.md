...
**浏览器自动化（Playwright / Vitest 组件渲染等）是项目测试体系的一等公民，可用于组件渲染测试与端到端（E2E）集成测试；但必须以受控、声明式、可复现的方式使用，严禁沦为临时探索调试工具而飞速消耗 Token。**
...
- **问题说明**：开发者常态使用代理软件的 **TUN (虚拟网卡) 模式**，导致浏览器自动化在请求外部 CDN（如 Google Fonts）时极易发生连接重置并死锁卡顿。
- **应对指导**：使用浏览器自动化测试时，必须严格遵守核心行为准则四的技术约束（本地静态化资源、超时上限、国内镜像下载）；严禁加载境外 CDN 资源，优先使用本地静态化资源并缩短超时等待。
...
# 🚨 核心行为准则十：AI 协作物理隔离开发实操流程
**在进行任何新微服务、中间件或业务插件开发时，AI 助手与开发人员必须遵循以下物理隔离开发与 TDD 流程，彻底杜绝全盘扫描造成的上下文冗余与代码污染：**

### 1. 物理隔离建档
* **唯一物理阵地**：在新服务开发前，必须在项目特定的位置创建一个**全新、干净**的 TypeScript 文件，例如 `src/kernel/services/QuotaCheckService.ts`。所有新逻辑必须在此隔离沙盒中编写，禁止直接侵入和修改 `Kernel.ts` 容器底座或其他无关服务。
...
### 3. TDD 单兵测试驱动
* **局部跑通验证**：在 `tests/run_all_tests.ts` 中追加独立的单元测试。通过在命令行执行 `npm run lint` 和 `npm run test` 进行局部单兵验证，在这个隔离测试容器中修补逻辑直至其 100% 成功通过。
