...
项目通过 [AGENTS.md](../AGENTS.md) 定义了 11 条核心行为准则,其中与代码审查最相关的有:

- **准则一**:超大规模扩展性与极致底座解耦(大单体防御、数据物理分轨)
- **准则二**:SillyTavern 生态兼容与底层原则(无侵入、数据驱动)
- **准则三**:纯移动端战略与原生适配(Safe Area、键盘避让、状态栏同步)
- **准则八**:AI 协作物理隔离开发铁律
- **准则十一**:云端后端与移动端物理隔离(`cloud/` 不打入 APK)
