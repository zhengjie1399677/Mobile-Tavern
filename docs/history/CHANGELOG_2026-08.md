...
- 2026-08-29：质量门禁与测试输出静默化：Vitest 非 TTY 改用自定义静默 reporter（仅失败与汇总，终端交互不变）、系统测试仅打印失败与汇总；pre-push 成功不再刷屏，完整输出落 `node_modules/.cache/quality-push.log`，失败回显尾部 100 行。
...
- 2026-08-29：会话激活改为可确认结果的原子流程，归档会话必须恢复后才能继续；聊天最近会话与平行宇宙改从权威目录按角色分页读取，避免角色错配和首屏快照漏项。
