...
export default function ChatHistoryTab() {
  ...
  // P1-12: 将原渲染中每个 session 三次 reduce 计算总字符数的开销，下沉到 useMemo 中
  // 仅按最后活动时间排序、并预计算每个 session 的派生数据（角色/最后消息/总字数展示串），
  // 仅在 sessions 或 characters 变化时重算，避免每次渲染 O(N*M) reduce。
    ...
    return [...sessions]
      .map((s) => {
        ...
        const totalCharsDisplay = totalChars > 1000
          ...
          : String(totalChars);
        return { s, char, lastMsg, lastActiveTime, totalCharsDisplay };
      })
