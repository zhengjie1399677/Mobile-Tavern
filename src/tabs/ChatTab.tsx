          ...
          <div className="flex flex-col flex-1 min-h-0 bg-background">
            ...
            <div className="bg-card p-3 border-b border-border flex items-center justify-between sticky top-0 z-10">
              <div className="flex items-center gap-2.5 min-w-0">
                <button onClick={() => setActiveTab("chat-history")} className="text-muted-foreground hover:text-foreground">
                ...
                <div className="w-10 h-10 rounded-lg overflow-hidden bg-muted border border-border/80 flex items-center justify-center">
                  {activeCharacter?.avatar ? (
                    ...
                    <span className="text-base font-serif font-bold text-primary">{activeCharacter?.name?.[0]}</span>
                  )}
                </div>
              ...
              <div className="flex bg-muted p-0.5 rounded-lg border border-border">
                <button
                  ...
                  className={`px-2.5 py-1 text-[11px] rounded transition font-medium flex items-center gap-1 ${
                    chatSubTab === "dialogue" ? "bg-primary/40 text-primary" : "text-muted-foreground hover:text-foreground"
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-2.5 py-1 text-[11px] rounded transition font-medium flex items-center gap-1 ${
                    chatSubTab === "timeline" ? "bg-primary/40 text-primary" : "text-muted-foreground hover:text-foreground"
                  }`}
                >
                  <History className="w-3.5 h-3.5" /> 故事年表 ({activeSession?.summaries?.length || 0})
                </button>
              </div>
            </div>
              ...
              <div className="flex-1 flex flex-col min-h-0">
                ...
                {activeSession && activeCharacter && (
                  <div className="bg-muted/50 border-b border-border px-3.5 py-1.5 flex items-center gap-2 overflow-x-auto text-[10.5px]">
                    <span className="text-primary font-medium whitespace-nowrap">世界书激活项:</span>
                    {(() => {
                      const textString = activeSession.messages.slice(-3).map((m)=>m.content).join(" ");
                      const triggered = (activeCharacter.lorebookEntries || []).filter(
                        (e) => e.enabled && (e.constant || e.keys.some((k) => k && textString.includes(k)))
                      );
                      if (triggered.length === 0) return <span className="text-muted-foreground italic">暂无事件激活</span>;
                      return triggered.map((t) => (
                        <span key={t.id} className="bg-muted border border-border text-muted-foreground px-1 py-0.5 rounded whitespace-nowrap" title={t.content}>
                          🔑 {t.keys[0]}
                        </span>
                      ));
                    })()}
                ...
                <div className="p-3.5 space-y-4 flex-1 overflow-y-auto custom-scrollbar">
                  {(() => {
                    ...
                    return (
                      <div key={message.id} className={`flex items-start gap-2.5 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
                        ...
                        {/* Speech Bubble */}
                        <div className="max-w-[78%] group relative" onClick={() => {
                          if (editingMsgId !== message.id) {
                            setMsgMenuId(msgMenuId === message.id ? null : message.id);
                          }
                        }}>
                          {editingMsgId === message.id ? (
                            <div className={`rounded-xl p-3 shadow-sm text-sm border transition-all ${
                              isUser
                              ...
                              <textarea
                                ...
                                rows={Math.max(2, editingMsgContent.split('\n').length)}
                                autoFocus
                              />
                              <div className="flex gap-2 justify-end">
                                <button
                                  onClick={async (e) => {
                                    e.stopPropagation();
                                    if (!activeSession) return;
                                    const nextMsgs = activeSession.messages.map((m) => m.id === message.id ? { ...m, content: editingMsgContent } : m);
                                    const updated = { ...activeSession, messages: nextMsgs };
                                    setSessions((prev) => prev.map((s) => s.id === updated.id ? updated : s));
                                    await saveSession(updated);
                            ...
                            <div className={`rounded-xl px-3.5 py-2.5 shadow-sm text-sm border font-light tracking-wide transition-all cursor-pointer ${
                              isUser
                                ? "bg-primary text-primary-foreground border-primary/50 hover:bg-primary/90"
                                : "bg-card text-foreground border-border shadow-sm active:scale-[0.98]"
                            }`}>
                              {message.content === "💭..." ? <TypingIndicator /> : renderDialogueBubble(message.content)}
                            </div>
                          )}
                          
                          {/* Bubble timestamp */}
                          <div className={`text-[10px] text-muted-foreground font-mono mt-1 ${isUser ? "text-right" : "text-left"} flex gap-2 ${isUser ? "justify-end" : "justify-start"} flex-wrap`}>
                            ...
                            {message.tokenCount !== undefined && message.tokenCount > 0 && (
                              <span className="flex items-center gap-1 opacity-70 border-l border-border pl-2" title={`提示词Tokens: ${message.promptTokenCount || 0}`}>
                          ...
                          {msgMenuId === message.id && (
                            <div className={`absolute top-full mt-1.5 bg-popover text-popover-foreground border border-border rounded-lg p-1.5 flex items-center gap-1 shadow-2xl z-10 ${
                              ...
                              <button
                                onClick={async (e) => {
                                  ...
                                  if (ok) {
                                    const nextMessages = activeSession.messages.filter((m) => m.id !== message.id);
                                    const updated = { ...activeSession, messages: nextMessages };
                                    setSessions((prev) => prev.map((s) => s.id === updated.id ? updated : s));
                                    await saveSession(updated);
                                    setMsgMenuId(null);
                                  }
            ...
            {chatSubTab === "timeline" && (
              <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4 min-h-0">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-sm font-bold text-foreground">故事历史卡片轴 (Memory Timeline)</h3>
                    <p className="text-[11px] text-muted-foreground">这些卡片将作为辅助长期记忆状态，拼写入系统 Prompt 中。</p>
                  </div>
                  <button
                    onClick={() => {
                      setNewSummaryTag(`幕段 ${sessions.find((s)=>s.id === activeSessionId)?.summaries?.length || 0}`);
                      setNewSummaryLoc(activeCharacter?.scenario?.slice(0, 8) || "荒野野营");
                ...
                <div className="relative border-l border-amber-655 border-primary/25 ml-3 pl-5 space-y-5 py-2">
                  {activeSession?.summaries.map((summary) => (
                    <div key={summary.id} className="relative group bg-card p-3 rounded-lg border border-border shadow-sm">
                      ...
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className="text-muted-foreground hover:text-muted-foreground p-0.5 text-[10px] bg-muted border border-border px-1 py-0.5 rounded flex items-center gap-0.5"
                          >
                          ...
                          <button
                            onClick={async () => {
                              if (!activeSession) return;
                              const ok = await showCustomConfirm("是否彻底解散清除该记忆卡片？");
                              if (ok) {
                                const nextSums = activeSession.summaries.filter((s) => s.id !== summary.id);
                                const updated = { ...activeSession, summaries: nextSums };
                                setSessions((prev) => prev.map((s) => s.id === updated.id ? updated : s));
                                await saveSession(updated);
                              }
                            }}
                  ...
                  {(!activeSession?.summaries || activeSession.summaries.length === 0) && (
                    <div className="text-center py-8 text-muted-foreground border border-dashed border-border/80 rounded pl-2">
                      ...
                      <p className="text-[10px] leading-normal px-4 mt-1 opacity-70">当聊天内容变长时，可通过下方 “智能记忆压缩” 自主浓缩，或手工录入您对当前关系演变的阶段性理解记录。</p>
                    </div>
                  )}
                </div>
              </div>
