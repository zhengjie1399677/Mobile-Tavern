      ...
      const schema: any = {
        ...
        parse(val: any) {
          if (val === undefined || val === null) {
            ...
            if (this._type === 'object') {
              ...
              if (this._shape) {
                for (const [key, subSchema] of Object.entries(this._shape)) {
                  res[key] = (subSchema as any).parse(undefined);
                }
              }
              return res;
            }
            ...
            if (this._type === 'number') return 0;
            if (this._type === 'boolean') return false;
            if (this._type === 'array') return [];
            if (this._type === 'enum' && Array.isArray(this._shape) && this._shape.length > 0) {
              return this._shape[0];
            }
            if (this._type === 'record' || this._type === 'map') return {};
            return undefined;
          }
          if (this._type === 'object' && this._shape) {
            const res: any = { ...val };
            for (const [key, subSchema] of Object.entries(this._shape)) {
