 ...
 *
 * 本文件已冻结，仅作为旧版外部导入兼容与测试重置入口。仓库内生产调用已经清零；
 * 严禁新增导出或生产依赖。兼容观察期结束且确认无外部调用后，应删除本文件。
 * 业务代码必须按职责使用应用 Service 或领域端口；Service 实现再调用具体仓库与适配器。
 * 存储实现本身严禁反向导入本文件，避免形成“实现 → 门面 → 实现”的依赖环。
 *
