...
export function useUsageTracking() {
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      let currentTodayRecord = currentMetrics.history.find(h => h.date === currentTodayStr);
    ...
    <Card className="bg-card border-border shadow-sm mt-4">
      <CardHeader className="pb-3 border-b border-border/50">
        <CardTitle className="text-sm flex items-center gap-2">
          <Activity className="w-4 h-4 text-emerald-500" /> 使用统计 (Telemetrics)
        </CardTitle>
        <CardDescription className="text-[11px]">基础运行信息（便于后续数据追踪打包需求）</CardDescription>
      </CardHeader>
      <CardContent className="pt-4 flex flex-col gap-3">
        <div className="flex items-center justify-between bg-muted/30 p-2 rounded border border-border">
          <span className="text-xs font-semibold text-foreground">累计打开次数</span>
          <span className="text-xs text-muted-foreground font-mono">{metrics.totalOpens} 次</span>
        </div>
        <div className="flex items-center justify-between bg-muted/30 p-2 rounded border border-border">
          <span className="text-xs font-semibold text-foreground">累计运行时间</span>
          <span className="text-xs text-muted-foreground font-mono">{Math.floor(metrics.totalUsageSeconds / 60)} 分钟</span>
        </div>
      </CardContent>
    </Card>
  );
