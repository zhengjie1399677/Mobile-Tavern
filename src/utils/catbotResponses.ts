 ...
 * 小猫助手雪团的本地预设吐槽词条数据
 *
 * 从 useCatbot.ts 抽离的纯静态数据模块，便于未来从外部 JSON 加载覆盖。
 * 数据结构：{ event_type: string[] | object }
 */
export const DEFAULT_CAT_RESPONSES = {
  "idle_click": [
    "喵？找本喵有什么事情嘛？",
    "酒馆今天也很热闹呢，要不要找个角色聊聊？",
    "呼咪……被你戳醒了喵！",
...
/**
 * 内存中的响应缓存，可被外部 JSON 加载覆盖。
 * 从 useCatbot.ts 抽离，使状态管理更内聚。
 */
let responsesCache: any = DEFAULT_CAT_RESPONSES;

export function getResponsesCache(): any {
  return responsesCache;
}

export function setResponsesCache(data: any): void {
  responsesCache = data;
}
