...
export default function App() {
  ...
  const handleAutoSummaryCheck = async (
    ...
    const interval =
      summaryTriggerTurns === 0 ? recentTurns : summaryTriggerTurns;
    ...
    if (force || session.messages.length >= maxAllowedMessages) {
      if (session.messages.length <= 2) {
        if (force)
          await showCustomAlert("当前历史消息不够，无法形成记忆碎片。");
        return;
      }

      const messagesToCompress =
        force && session.messages.length < maxAllowedMessages
      ...
      try {
        const promptInstruction =
          "你是一个精简的大纲压缩器。请用极简的语句，将以下角色扮演的对话梗概总结为一条日记式故事时间轴记忆，格式必须如：'[时间状态(如“临晨”)] 总结内容'。字数在150字以内。";
