...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<
    "characters" | "chat" | "chat-history" | "settings" | "global-worldbook"
  >("characters");
  const [chatSubTab, setChatSubTab] = useState<"dialogue" | "timeline">(
    "dialogue",
  );
  ...
  const [editingChar, setEditingChar] = useState<Partial<CharacterCard> | null>(
    null,
  );
  ...
  const [activeLoreTab, setActiveLoreTab] = useState<"detail" | "lore">(
    "detail",
  );
  const [editingLoreEntry, setEditingLoreEntry] =
    useState<Partial<LorebookEntry> | null>(null);
  const [expandedLoreIds, setExpandedLoreIds] = useState<
    Record<string, boolean>
  >({});

  // Independent active-character lorebook editor (Tab version)
  const [editingActiveCharLoreEntry, setEditingActiveCharLoreEntry] =
    useState<Partial<LorebookEntry> | null>(null);

  // Active selected host in Worldbook Tab
  const [activeWorldbookHostId, setActiveWorldbookHostId] =
  ...
  const [activeSettingAccordion, setActiveSettingAccordion] = useState<
    string | null
  >("api");
  const [sillyInnerTab, setSillyInnerTab] = useState<"samplers" | "prompts">(
    "samplers",
  );
  ...
  useEffect(() => {
    ...
    reportUsage("app_launch");

    // Telemetry: App Close
    const handleBeforeUnload = () => {
      reportUsage("app_close");
    };
    ...
    reader.onload = async (event) => {
      try {
        ...
        let mainPrompt =
          ...
          data.systemPrompt ||
          data.mainPrompt ||
          data.system_sequence ||
          "";
        let jailbreakPrompt =
          data.jailbreak_prompt || data.jailbreak || data.jailbreakPrompt || "";
        let postHistoryPrompt =
          data.post_history_instructions ||
          data.post_history_prompt ||
          data.character_bias ||
          data.postHistoryPrompt ||
          "";
        let storyStrFromJSON =
          data.story_string ||
          data.storyString ||
        ...
        if (data.prompts && Array.isArray(data.prompts)) {
          ...
          const mainItem = data.prompts.find(
            (p: any) =>
              p.identifier === "main" ||
              p.name === "初始化" ||
          ...
          const jbItem = data.prompts.find(
            (p: any) =>
        ...
        const instructTemplate =
          data.instruct_template ||
          data.instructTemplate ||
          (data.user_sequence || data.assistant_sequence
            ? "custom"
            : "default");
        const systemPrefix =
          data.system_sequence_start ||
          data.systemPrefix ||
          data.system_sequence ||
          "";
        const systemSuffix =
          data.system_sequence_end || data.systemSuffix || "";
        const userPrefix =
          data.user_sequence_start ||
          data.userPrefix ||
          data.user_sequence ||
          "";
        const userSuffix = data.user_sequence_end || data.userSuffix || "";
        const assistantPrefix =
          data.assistant_sequence_start ||
          data.assistantPrefix ||
          data.assistant_sequence ||
          "";
  ...
  const createNewSessionOfCharacter = async (
    charId: string,
    title?: string,
  ) => {
    const targetChar = characters.find((c) => c.id === charId);
    ...
    const newSession: ChatSession = {
      ...
      title:
        title ||
        `全新旅程 (${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString()})`,
      createdAt: Date.now(),
      messages: targetChar.first_mes
        ? [
            {
  ...
  const handleRerollFromMessage = async (targetMsg: Message) => {
    ...
    try {
      ...
      // Telemetry: track message regenerate with token usage
  ...
  const handleAutoSummaryCheck = async (
    ...
    const { summaryLength } = settings.memory;
    
    // Check what hasn't been summarized yet
    ...
    if (force || unsummarizedMessages.length >= maxAllowedUnsummarized) {
      if (unsummarizedMessages.length === 0) {
        if (force)
          await showCustomAlert("当前没有未被总结的有效对话。");
        return;
      }

      // Max 20 sentences (messages). 无论是手动还是自动，最多只提取20条
      ...
      try {
        ...
        if (compiledSummary) {
          ...
          // Get the ID of the last message that was grouped into this summary
          ...
          await saveSession(finalSession);
          if (force)
            await showCustomAlert("记忆整理完毕，已收录至潜意识年表！");
        } else {
        ...
        if (force)
          await showCustomAlert("记忆整理出错: " + (e as Error).message);
      }
    } else {
    ...
    try {
      ...
      if (data.success) {
        setConnectionStatus({
          testing: false,
          success: true,
          message: data.message || "测试通过，连接就绪！",
        });
      } else {
        setConnectionStatus({
          testing: false,
          ...
          if (stPosition !== undefined) {
            ...
            if (!isNaN(numPos)) {
              switch (numPos) {
                ...
                case 2: position = "after_char_def"; break;  // ST before AN / after scenario -> fallback after_char_def
                case 3: position = "after_char_def"; break;  // ST before chat -> fallback after_char_def
  ...
  const handleImportLocalDataBackup = async (
    ...
    try {
      ...
      if (!parsed.characters || !parsed.sessions) {
        throw new Error(
          "解密所得的索引列表无效，非此程序认可的合法数据组织结构。",
        );
      }

      // Sync both state inside memory and persistent engine
      const ok = await showCustomConfirm(
        "数据解密成功！此备份覆盖将导致当前浏览器的本地全部状态清空，是否确认还原？",
      );
      if (ok) {
        ...
        if (parsed.globalLorebook)
          await saveGlobalLorebook(parsed.globalLorebook);
  ...
  const createBacktrackFromTimeline = async (summary: SummaryCard) => {
    if (!activeCharacter || !activeSession) return;
    const sumIdx = activeSession.summaries.findIndex(
      (s) => s.id === summary.id,
    );
    if (sumIdx < 0) return;

    // A branch starting with only summaries up to that index, and a blank slate or original greeting
    const targetBranchesSummaries = activeSession.summaries.slice(
      0,
      sumIdx + 1,
    );
    const branchTitle = await showCustomPrompt(
  ...
  const handleAddTimelineSummary = async () => {
    if (!newSummaryTag.trim() || !newSummaryContent.trim() || !activeSession)
      return;
  ...
  const handleSaveActiveCharLoreEntry = async () => {
    ...
    const keysArr = Array.isArray(editingActiveCharLoreEntry.keys)
      ...
      : (editingActiveCharLoreEntry.keys as unknown as string)
          .split(",")
          .map((k) => k.trim())
          .filter(Boolean);
  ...
  const [editingGlobalEntry, setEditingGlobalEntry] =
    useState<Partial<LorebookEntry> | null>(null);
  const handleSaveGlobalLoreEntry = async () => {
    ...
    const keysArr = Array.isArray(editingGlobalEntry.keys)
      ...
      : (editingGlobalEntry.keys as unknown as string)
          .split(",")
          .map((k) => k.trim())
          .filter(Boolean);
    ...
    const newEntry: LorebookEntry = {
      id:
        editingGlobalEntry.id ||
        "glo_" + Math.random().toString(36).substring(2, 9),
      keys: keysArr,
      ...
      probability:
        editingGlobalEntry.probability !== undefined
          ? Number(editingGlobalEntry.probability)
          : 100,
      order:
        editingGlobalEntry.order !== undefined
          ? Number(editingGlobalEntry.order)
          : 100,
      position: editingGlobalEntry.position || "after_char_def",
      depth:
        editingGlobalEntry.depth !== undefined
          ? Number(editingGlobalEntry.depth)
          : 4,
    };
  ...
  const appContextValue = {
    ...
    editingGlobalEntry,
    setEditingGlobalEntry,
    handleSaveGlobalLoreEntry,
            ...
            <div className="bg-background border-t border-border max-h-[92%] overflow-y-auto rounded-t-2xl flex flex-col">
              ...
              {activeLoreTab === "detail" && (
                <div className="p-4 space-y-3.5 text-xs">
                  ...
                  <div>
                    <label className="block text-muted-foreground mb-1">
                      自定义系统提示约束 (System Instruction constraint
                      Override)
                    </label>
                    <input
                      type="text"
                              ...
                              {!isEditingThis ? (
                                <>
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    <p
                                      className={`font-light leading-relaxed whitespace-pre-wrap rounded-lg bg-muted/40 p-2 border border-border/30 text-[11px] ${entry.disabled ? "line-through text-muted-foreground/50" : "text-muted-foreground"}`}
                                    >
                                      {entry.content}
                                    </p>
                                  </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-end gap-2.5 pt-1.5">
                  ...
                  <button
                    ...
                    disabled={
                      !newSummaryTag.trim() || !newSummaryContent.trim()
                    }
              ...
              <div className="flex-1 overflow-y-auto space-y-2 pb-4 pr-1 custom-scrollbar">
                {sessions
                  ...
                  .map((s) => (
                    <div
                      key={s.id}
                      className={`p-3 border rounded-lg flex flex-col gap-2 transition-colors cursor-pointer ${s.id === activeSession?.id ? "border-primary bg-primary/10" : "border-border bg-card hover:border-primary/50"}`}
                      onClick={() => {
