...
export async function registerMainTabExtensions(kernel: IKernel): Promise<void> {
  ...
  if (await shouldShowCommunityEntry()) {
    kernel.registerExtension({ id: "community", targetPoint: "main:tabs", priority: 95, component: CommunityTab, meta: { name: "社区", icon: "Users", showInBottomBar: true } });
  }
