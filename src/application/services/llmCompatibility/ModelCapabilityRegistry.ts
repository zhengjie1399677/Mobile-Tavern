...
export type { ProviderFamily } from "./types";
...
const RUNTIME_CACHE_STORAGE_KEY = 'mt_model_capability_runtime_cache_v3';
...
export class ModelCapabilityRegistry {
  ...
  private static readonly KNOWN_STANDARD_PROVIDERS = [
    ...
    "lepton.ai",              // Lepton AI
    "newapi.pro",             // New API 托管网关
    "oneapi.com",             // One API 托管网关
    "one-api.com",            // One-API 托管网关
  ];
  ...
  private static isStandardProvider(baseUrl?: string): boolean {
    ...
    return this.KNOWN_STANDARD_PROVIDERS.some((domain) => {
  ...
  static getCapabilities(modelId: string, baseUrl?: string): ModelCapabilities {
    ...
    if (caps.supportsStreamOptions === undefined) {
      const identity = resolveProviderIdentity(baseUrl, modelId);
      caps.supportsStreamOptions = identity.official
        && (identity.family === "openai" || identity.family === "deepseek");
      if (!caps.supportsStreamOptions && baseUrl) {
        try {
          const hostname = new URL(baseUrl).hostname.toLowerCase();
          caps.supportsStreamOptions = hostname === "openrouter.ai" || hostname.endsWith(".openrouter.ai");
        } catch {
    ...
    const cached = this.runtimeCache.get(this.getRuntimeCacheKey(modelId, baseUrl));
    if (cached) {
   ...
   */
  static resolveProviderFamily(baseUrl?: string, modelId = ""): ProviderFamily {
    return resolveProviderIdentity(baseUrl, modelId).family;
  }
  ...
  static updateCapabilities(
    ...
    // 仅允许关闭能力，不允许开启
    const cacheKey = this.getRuntimeCacheKey(modelId, baseUrl);
    const next: Partial<ModelCapabilities> = { ...(this.runtimeCache.get(cacheKey) ?? {}) };
    ...
    this.runtimeCache.set(cacheKey, next);
   ...
   */
  static isUnsupportedParamError(error: unknown): UnsupportedProviderParameter | null {
    const errorMsg = getUnknownErrorText(error);
    ...
    const paramPatterns: Array<{
      ...
      { capability: 'supportsTopK', requestFields: ['top_k'], pattern: /top_k|topK/i },
      { capability: 'supportsTopP', requestFields: ['top_p'], pattern: /top_p|topP/i },
      { capability: 'supportsTemperature', requestFields: ['temperature'], pattern: /temperature/i },
      { capability: 'supportsJsonSchema', requestFields: ['response_format', 'json_schema'], pattern: /response_format|json_schema/i },
      { capability: 'supportsFunctionCalling', requestFields: ['tools', 'tool_choice', 'functions', 'function_call'], pattern: /function_call|tool_choice|tools\b/i },
      { capability: 'supportsMinP', requestFields: ['min_p'], pattern: /min_p|minP/i },
      { capability: 'supportsRepetitionPenalty', requestFields: ['repetition_penalty'], pattern: /repetition_penalty|repetitionPenalty|rep_pen/i },
    ...
    for (const { capability, requestFields, pattern } of paramPatterns) {
      if (pattern.test(errorMsg)) {
        return { capability, param: capability, requestFields };
      }
    }
    return null;
  }
  ...
  private static getRuntimeCacheKey(modelId: string, baseUrl?: string): string {
    const identity = resolveProviderIdentity(baseUrl, modelId);
    return `${identity.scopeKey}::${modelId.trim().toLowerCase()}`;
  }
}
...
function getUnknownErrorText(error: unknown): string {
  ...
  for (const key of ["message", "statusText", "error", "detail"]) {
