...
import { IKernel, IDatabaseService, ILLMService, KernelServices } from "../serviceContracts";
...
export class AutoSummaryService {
  ...
  async handleAutoSummaryCheck(
    ...
    if (force || unsummarizedCount >= maxAllowedUnsummarized) {
      ...
      // 后台任务：trial 配额耗尽时静默跳过，不抛 alert。
      let creds;
      try {
        creds = resolveApiCredentials(settings);
      } catch (e) {
        if (e instanceof TrialExhaustedError) {
          logger.info("Trial quota exhausted, skip summary", { sessionId: session.id });
          return session;
        }
        throw e;
      }
