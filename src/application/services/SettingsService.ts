import { IKernel, ISettingsService } from "../serviceContracts";
import { UserSettings } from "../../types";
import {
  getStoredSettings as dbGetStoredSettings,
  saveStoredSettings as dbSaveStoredSettings,
  getStoredUsageMetrics as dbGetStoredUsageMetrics,
  saveStoredUsageMetrics as dbSaveStoredUsageMetrics,
 ...
 */
export class SettingsService implements ISettingsService<UserSettings, UsageMetrics> {
  name = "settings";
  isCritical = false;
  ...
  async getUsageMetrics(): Promise<UsageMetrics | null> {
    return (await dbGetStoredUsageMetrics()) as UsageMetrics | null;
  }

  async saveUsageMetrics(metrics: UsageMetrics): Promise<void> {
    return dbSaveStoredUsageMetrics(metrics);
  }
}
