// tavern-ar Native Adapter
//
// 通过 Tauri invoke 调用 Kotlin @Command 方法，适配到全屏 ArActivity。
// 桌面环境（非 Android WebView）下 invoke 会失败，所有方法都做 try/catch 兜底。
// 本文件只适配 Tauri/Kotlin AR 命令，不属于 Plugin Host RPC，
// 也不得复用 SillyTavern Compatibility Runtime。
//
